...
function assigneesOf(review) { return String(review.assignee || '').split(',').map(name => name.trim()).filter(Boolean); }
...
function App() {
  ...
  const addReview = (review) => {
    const saved = { id: crypto.randomUUID(), key: nextTaskKey(data.reviews), createdAt: new Date().toISOString().slice(0, 10), archived: false, ...review };
    if (!saved.key || !/^AR-\d+$/.test(saved.key)) saved.key = nextTaskKey(data.reviews);
    setData(prev => ({ ...prev, reviews: [saved, ...prev.reviews] }));
    api('/api/reviews', { method: 'POST', body: JSON.stringify(saved) }).then(created => {
    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className="notification" aria-label="Notifications" onClick={()=>{setNotificationsOpen(open=>!open);if(!notifications.length)refreshNotifications()}}><Bell size={17}/><i/></button>
        </div>
      </header>
      {query && <GlobalSearchResults query={query} reviews={activeReviews} onOpen={setSelectedReview} onClear={()=>setQuery('')}/>}
      {notificationsOpen && <NotificationMenu notifications={notifications} onClose={()=>setNotificationsOpen(false)} onOpen={setSelectedReview}/>}
...
function NotificationMenu({ notifications, onClose, onOpen }) {
  return <div className="notification-menu"><div className="search-results-head">Recent activity <button onClick={onClose}><X size={14}/></button></div>{notifications.length ? notifications.slice(0, 8).map(item => <button key={item.id} onClick={()=>{onOpen({id:item.reviewId,title:item.title});onClose()}}><span className="notification-icon"><Bell size={14}/></span><span><strong>{item.title}</strong><small>{item.name || 'System'} {item.action} · {relativeDate(item.createdAt)}</small></span></button>) : <p>No notifications yet.</p>}</div>;
}
...
function Dashboard({ reviews, meetings, goTo, onSubmitReview, onNewMeeting }) {
  ...
  return <section className="page dashboard-page">
    ...
    <div className="dashboard-grid">
      ...
      <div className="dashboard-rail"><MeetingRail meetings={meetings} goTo={goTo} onNew={onNewMeeting}/><WorkloadCard reviews={reviews}/><Summary reviews={reviews} completed={completed} blockers={blockers} overdue={overdue}/></div>
    </div>
  </section>;
}
...
function WorkloadCard({ reviews }) {
  const rows = useMemo(() => {
    const counts = new Map();
    reviews.filter(r => !r.archived && r.stage !== 'Completed').forEach(r => assigneesOf(r).forEach(name => counts.set(name, (counts.get(name) || 0) + 1)));
    return [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 6);
  }, [reviews]);
  if (!rows.length) return null;
  return <aside className="rail-card summary"><h2>Workload</h2><p>Active tasks per person</p><div className="summary-list">{rows.map(([name, count]) => <Metric key={name} label={name} value={count}/>)}</div></aside>;
}
...
function Reviews({ reviews, query, setQuery, updateReview, archiveReview, setModal, onOpen }) {
  ...
  const today = new Date().toISOString().slice(0, 10);
  const owners = useMemo(() => [...new Set(reviews.flatMap(r => assigneesOf(r)))], [reviews]);
  const filtered = reviews.filter(r =>
    (status === 'All statuses' || statusFor(r) === status) &&
    (priority === 'All priorities' || r.priority === priority) &&
    (owner === 'All owners' || assigneesOf(r).includes(owner)) &&
...
function ReviewDetailEnhanced({ review, meetings, user, onClose, onUpdated, onDeleted, onToast, onRemoveRequest }) {
  const [detail, setDetail] = useState(null); const [draft, setDraft] = useState(review); const [comment, setComment] = useState(''); const [subtask, setSubtask] = useState('');
...
function SettingsPageEnhanced({ project, user, saveProject, setToast }) {
...
function AdminPageEnhanced({ user, setToast }) {
  const [admins, setAdmins] = useState([]); const [error, setError] = useState(''); const [inviteOpen, setInviteOpen] = useState(false); const [invite, setInvite] = useState({ name: '', email: '', password: '' });
  const load = () => api('/api/admin/users').then(result => setAdmins(result.users)).catch(error => setError(error.message)); useEffect(load, []);
